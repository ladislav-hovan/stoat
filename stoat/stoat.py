...
class Stoat:
    ...
    def plot_spots(
        ...
        vcoord = -self.spatial['xInd']
        # Horizontal cartesian coordinates
        hcoord = (2 * np.sin(np.radians(60)) / 3) * self.spatial['yInd']
        ...
        if colour_from is not None:
            # Choose averaged or original data
            if averaged:
                plot_df = self.avg_expression
            else:
                plot_df = self.expression
            # Sum all the genes or use a specific one
            if colour_from == 'sum_all':
                colour_vals = plot_df.sum(axis=1)
            else:
        ...
        # Pick the validity Series based on whether we're using the averaged
        # expression or not
        if averaged:
            valid = self.spatial['Valid']
        else:
            valid = self.spatial['Success']
